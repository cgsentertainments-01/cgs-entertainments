-- =============================================================================
-- CGS ENTERTAINMENTS - EVENT RESULTS & CERTIFICATIONS SYNCHRONIZATION MIGRATION
-- =============================================================================
...
-- Safely ensure columns exist if table pre-existed
DO $$
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='event_results' AND column_name='notify_sent') THEN
        ALTER TABLE public.event_results ADD COLUMN notify_sent BOOLEAN NOT NULL DEFAULT false;
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_event_results_registration_id ON public.event_results(registration_id);
