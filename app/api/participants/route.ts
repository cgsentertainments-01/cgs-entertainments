...
export async function GET(request: Request) {
  try {
    ...
    let regQuery = supabase
      .from("registrations")
      .select("id, registration_number, event_id, participant_id, category_id, dance_style_id, registration_status, payment_status, registration_date, amount, notes, created_at")
    ...
    if (participantIds.length > 0) {
      const { data: participants } = await supabase
        .from("participants")
        .select("id, participant_number, full_name, email, phone, date_of_birth, gender, city, state, profile_photo, created_at")
